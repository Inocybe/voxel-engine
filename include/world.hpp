...
public:
    ...
    std::deque<std::tuple<int, int, int>> chunkVertexGenerationQueue; // queue of chunk coordinates to generate vertex data for, used to store the chunks that need to have their vertex data generated, so that they can be generated by the mesh worker thread
